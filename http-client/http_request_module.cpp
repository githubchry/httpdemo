...
static ry_pthread_t *uds_recv_thread_ptr = NULL;
//用来接收服务器一个buffer
...
static void *uds_recv_thread(ry_pthread_t *self)
{
    ...
    assert_param_return(sockfd >= 0, NULL);

    struct sockaddr_un server_addr = { 0 };
    server_addr.sun_family = AF_UNIX;
    assert_param_return(strlen(UDS_DGRAM_SERVER_PATH) < sizeof(server_addr.sun_path), NULL);
    ...
    assert_param_return(recv_buf_ptr, NULL);


    int addr_size = -1;
    struct sockaddr_un client_addr = { 0 };

    while (!ry_pthread_get_is_destroy(self))
    {
...
{
    uds_recv_thread_ptr = ry_pthread_create(0, uds_recv_thread, NULL);
}